...
public class _3StreamMapFilter {
...
//- whereas a Map allows you to define or use an existing function on the stream eg you can apply String.toUpperCase(...)
    public static void main(String[] args) {
        ...
        System.out.println(fruits.stream().map(fr -> fr.length()).toList()); // fruits.stream().map(fr -> String::length).toList() => [5, 5, 6, 4, 6, 6]
        ...
        System.out.println(fruits.stream().map(f -> f.length()).filter(length -> length > 5).toList()); // [6, 6, 6]
